  ...
  if (todoList.todoItems.length <= 0 && enableEmptyMessage) {
    todoItemsContainer.appendChild(
      elementCreator.getElement(
        "div",
        "Seems like there is nothing to display here yet",
        "empty-todo-items-container"
      )
    );
    return;
  }
    ...
    deleteButtonContainer.addEventListener("click", () => {
      ...
      if (todoItem.isCompleted) {
        ...
        elementForms.showDeleteForm(
          "Are you sure you want to delete this uncompleted Todo?<br> This can not be reverted.",
          () => {
            dataManager.deleteTodoItem(todoItem, projectTodoList);
            domManager.displaySelectedTab(globals.getSelectedTab());
          }
